...
безопасно завершает старый контекст и запускает новый с тем же локальным
профилем. Во время shutdown сигнал Ctrl+C временно блокируется, чтобы не
оставлять незавершённые внутренние задачи Playwright.
